...
const AuthForm: React.FC<AuthFormProps> = ({ initialMode = 'login' }) => {
  const navigate = useNavigate();
  const { login } = useAuth(); // Теперь функция login принимает токены
  const [mode, setMode] = useState<'login' | 'register'>(initialMode);
  const [loading, setLoading] = useState(false);
  const [locations, setLocations] = useState<Array<{id: number, city: string}>>([]);
  const [form] = Form.useForm<FormValues>();
  ...
  const onFinish = async (values: FormValues) => {
    ...
    try {
      if (mode === 'login') {
        console.log(' Начало логина с данными:', { 
          username: values.email,
        ...
        console.log(' Ответ от API логина:', {
        ...
        console.log(' Ответ от API регистрации:', {
          hasAccessToken: !!response.access_token,
          hasRefreshToken: !!response.refresh_token,
          message: response.message
        });
      ...
      console.error(' Ошибка в AuthForm:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError.response?.data?.message || 
                          apiError.response?.data?.detail || 
                          apiError.message || 
